...
class Hyperparameters:
    ...
    qk_gain_init = float(os.environ.get("QK_GAIN_INIT", 5.75))
    ...
    muon_wd = float(os.environ.get("MUON_WD", 0.098))
    embed_wd = float(os.environ.get("EMBED_WD", 0.088))
    ...
    matrix_clip_sigmas = float(os.environ.get("MATRIX_CLIP_SIGMAS", 13.0))
    embed_clip_sigmas = float(os.environ.get("EMBED_CLIP_SIGMAS", 21.0))
...
def gptq_mixed_quantize(state_dict, hessians, h):
    ...
    for cat in sorted(categories):
        joined = ", ".join(sorted(categories[cat]))
        log(f"  {cat}: {joined}")
    return result, meta
...
def train_and_eval(h, device):
    ...
    num_shards = len(list(Path(h.datasets_dir).resolve().glob('fineweb_train_*.bin')))
    log(f"train_shards: {num_shards}")
