...
def qat_snap_weights(model,clip_sigmas,clip_range,alpha):
	"""Snap weights toward quantized values. Called after optimizer step during warmdown."""
	with torch.no_grad():
		for module in model.modules():
			if isinstance(module,CastedLinear)and module.weight.numel()>65536:
				w=module.weight.float();row_std=w.std(dim=1,keepdim=True)
				s=(clip_sigmas*row_std/clip_range).clamp_min(1e-10)
				w_q=torch.clamp(torch.round(w/s),-clip_range,clip_range)*s
				module.weight.data.copy_((alpha*w_q+(1.-alpha)*w).to(module.weight.dtype))
...
def train_model(h,device,val_data):
	...
	while True:
		...
		if h.qat_enabled and frac>=h.qat_start_frac:
			qat_progress=min((frac-h.qat_start_frac)/(1.-h.qat_start_frac),1.);qat_alpha=h.qat_alpha_final*qat_progress
			qat_snap_weights(base_model,h.matrix_clip_sigmas,2**(h.matrix_bits-1)-1,qat_alpha)
		with torch.no_grad():
