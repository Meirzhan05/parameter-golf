...
class DistributedTokenLoader:
	...
	def __init__(self, h, device):
		...
		self.device = device
		self.files = [Path(p) for p in sorted(glob.glob(h.train_files))]
		if not self.files:
		...
		self._cfg = None
		self._eligible_shards = None
		self._base_block_counts = None
		n = len(self.files)
	...
	def _reset_cursor(self, si, seq_len):
	...
	def _init_pipeline(self, global_tokens, seq_len, grad_accum_steps):
	...
	def _sample_global_windows(self):
		...
		for si, cnt in zip(cs.tolist(), counts.tolist()):
			b = []
			self._take_from_shard(int(si), self._cfg[1], int(cnt), b)
			if b:
				if len(b) > 1:
...
class CausalSelfAttention(nn.Module):
	def __init__(self,dim,num_heads,num_kv_heads,rope_base,qk_gain_init,train_seq_len,attn_out_gate_width=0):
		super().__init__()
		...
		self.attn_out_gate_width=attn_out_gate_width
		if attn_out_gate_width>0:self.attn_gate_proj=CastedLinear(attn_out_gate_width,num_heads,bias=False);self.attn_gate_proj._zero_init=True
		else:self.attn_gate_proj=None
	...
	def forward(self,x):
		...
		if self.attn_gate_proj is not None:gate_in=x[...,:self.attn_out_gate_width].contiguous();g=2.*torch.sigmoid(self.attn_gate_proj(gate_in));y=y*g[...,None]
...
class GPT(nn.Module):
	def __init__(self,h):
		...
		self.tie_embeddings=h.tie_embeddings;self.tied_embed_init_std=h.tied_embed_init_std;self.logit_softcap=h.logit_softcap;self.logit_rescale_mode=h.logit_rescale_mode;self.logit_rescale_scale=h.logit_rescale_scale;self.logit_rescale_offset=h.logit_rescale_offset;self.logit_rescale_steepness=h.logit_rescale_steepness;self.tok_emb=nn.Embedding(h.vocab_size,h.embedding_dim)
		...
		self.num_encoder_layers=h.num_layers//2;self.num_decoder_layers=h.num_layers-self.num_encoder_layers;aogw=h.attn_out_gate_width if h.attn_out_gate_enabled else 0;self.blocks=nn.ModuleList([Block(h.model_dim,h.num_heads,h.num_kv_heads,h.mlp_mult,h.rope_base,h.qk_gain_init,h.train_seq_len,layer_idx=i,ln_scale=h.ln_scale,attn_out_gate_width=aogw)for i in range(h.num_layers)])
		if h.rope_dims>0:
		...
		self.smear_gate_width=h.smear_gate_width if h.smear_gate_enabled else 0;self.smear_bos_token_id=h.smear_bos_token_id
		if self.smear_gate_width>0:self.smear_gate=CastedLinear(self.smear_gate_width,1,bias=False);self.smear_gate._zero_init=True;self.smear_lambda=nn.Parameter(torch.zeros(1,dtype=torch.float32))
		else:self.smear_gate=None;self.smear_lambda=None
		self._init_weights()
	...
	def forward_logits(self,input_ids):
		...
		if self.smear_gate is not None:
			sl=self.smear_lambda.to(dtype=x.dtype);gate_in=x[:,1:,:self.smear_gate_width].contiguous();g=sl*torch.sigmoid(self.smear_gate(gate_in))
			bos_mask=(input_ids[:,1:]==self.smear_bos_token_id).unsqueeze(-1);g=g.masked_fill(bos_mask,0.)
		...
		for(pos,i)in enumerate(enc_iter):x=self.blocks[i](x,x0);skips.append(x)
		...
		if self.logit_rescale_mode=='sigmoid':logits=self.logit_rescale_scale*torch.sigmoid((logits_proj+self.logit_rescale_offset)/self.logit_rescale_steepness)
		else:logits=self.logit_softcap*torch.tanh(logits_proj/self.logit_softcap)
		return logits
...
class Muon(torch.optim.Optimizer):
	...
	def step(self,closure=None):
		...
		return loss
CONTROL_TENSOR_NAME_PATTERNS=tuple(pattern for pattern in os.environ.get('CONTROL_TENSOR_NAME_PATTERNS','attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,skip_gates,attn_gate,smear').split(',')if pattern)
class Optimizers:
	def __init__(self,h,base_model):
		...
		if base_model.smear_gate is not None:scalar_params.append(base_model.smear_gate.weight);scalar_params.append(base_model.smear_lambda)
...
def collect_hessians(model,train_loader,h,device,n_calibration_batches=64):
	...
	return hessians
def quantize_int6_per_row(W,clip_range=31):
	"""Simple per-row quantization fallback (no Hessian). Used when Cholesky fails."""
	t32=W.float()
	if t32.ndim==2:
...
def serialize(h,base_model,code):
	...
	sd_cpu={k:v.detach().cpu()for(k,v)in base_model.state_dict().items()};device=torch.device('cuda',h.local_rank);log('GPTQ:collecting Hessians from calibration data...');t0=time.perf_counter();calib_loader=DistributedTokenLoader(h,device);hessians=collect_hessians(base_model,calib_loader,h,device,n_calibration_batches=h.gptq_calibration_batches);log(f"GPTQ:collected {len(hessians)} Hessians in {time.perf_counter()-t0:.1f}s");quant_result,quant_meta=gptq_mixed_quantize(sd_cpu,hessians,h);quant_buf=io.BytesIO();torch.save({'w':quant_result,'m':quant_meta},quant_buf);quant_raw=quant_buf.getvalue();quant_blob=_compress(quant_raw,h.compressor);quant_file_bytes=len(quant_blob);bytes_total=quant_file_bytes+code_bytes
...
def deserialize(h,device):
	...
	quant_state=torch.load(io.BytesIO(_decompress(quant_blob_disk,h.compressor)),map_location='cpu');deq_state=dequantize_mixed(quant_state['w'],quant_state['m'],sd_cpu);eval_model.load_state_dict(deq_state,strict=False);return eval_model
...
def train_model(h,device,val_data):
	...
	log(f"model_params:{sum(p.numel()for p in base_model.parameters())}");optimizers=Optimizers(h,base_model);train_loader=DistributedTokenLoader(h,device);max_wallclock_ms=1e3*h.max_wallclock_seconds if h.max_wallclock_seconds>0 else None
	...
	if h.warmup_steps>0:
		...
		train_loader=DistributedTokenLoader(h,device)
	...
	log(f"peak memory allocated: {torch.cuda.max_memory_allocated()//1024//1024} MiB reserved: {torch.cuda.max_memory_reserved()//1024//1024} MiB");log('ema:applying EMA weights');current_state=base_model.state_dict();avg_state={name:t.to(dtype=current_state[name].dtype)for(name,t)in ema_state.items()}
	for name in current_state:
		if name not in avg_state:avg_state[name]=current_state[name]
