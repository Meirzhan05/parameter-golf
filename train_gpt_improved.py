...
def gptq_quantize_weight(w,H,clip_sigmas=3.,clip_range=63,block_size=128,hessian_lambda=0.175):
	"""GPTQ with Hessian-aware SD clipping, actorder, and Cholesky fallback."""
	W_orig=w.float().clone();rows,cols=W_orig.shape;H=H.float().clone();dead=torch.diag(H)==0;H[dead,dead]=1;damp=.01*H.diag().mean();H.diagonal().add_(damp)
	hdiag=H.diag().clone()
	perm=torch.argsort(hdiag,descending=True);invperm=torch.argsort(perm);W_perm=W_orig[:,perm].clone();W_perm[:,dead[perm]]=0;H=H[perm][:,perm]
	try:
	...
	if hessian_lambda>0:
		row_importance=(W_orig.pow(2)*hdiag[None,:]).sum(dim=1);ri=row_importance/(row_importance.mean()+1e-10)
		clip_mult=1.+hessian_lambda*(ri-1.);clip_k=clip_sigmas*clip_mult
	else:clip_k=clip_sigmas
	s=(clip_k*row_std/clip_range).clamp_min(1e-10).to(torch.float16);sf=s.float()
...
def gptq_mixed_quantize(state_dict,hessians,h):
	...
	for(name,tensor)in state_dict.items():
		...
		cs=h.embed_clip_sigmas if'tok_emb'in name else h.matrix_clip_sigmas;bits=h.embed_bits if'tok_emb'in name else h.matrix_bits;hl=0 if'tok_emb'in name else h.hessian_lambda;q,s=gptq_quantize_weight(t,hessians[name],clip_sigmas=cs,clip_range=2**(bits-1)-1,hessian_lambda=hl);result[name+'.q']=q;result[name+'.scale']=s;meta[name]=f"gptq (int{bits})"
