...
def gptq_mixed_quantize(state_dict,hessians,h):
	...
	for(name,tensor)in state_dict.items():
		...
		if'tok_emb'in name:cs=h.embed_clip_sigmas;bits=h.embed_bits
		else:
			bits=h.matrix_bits;cs=h.matrix_clip_sigmas
			m=re.search(r'blocks\.(\d+)\.',name)
			if m:
				li=int(m.group(1))
				if li<h.loop_start:cs*=h.clip_mult_early
				elif li<=h.loop_end:cs*=h.clip_mult_loop
				elif li<h.parallel_residual_start:cs*=h.clip_mult_mid
				else:cs*=h.clip_mult_late
		q,s=gptq_quantize_weight(t,hessians[name],clip_sigmas=cs,clip_range=2**(bits-1)-1);result[name+'.q']=q;result[name+'.scale']=s;meta[name]=f"gptq (int{bits})"
